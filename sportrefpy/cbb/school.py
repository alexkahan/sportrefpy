...
class CBBSchool(CBB):
    ...
    def school_summary(self):
        ...
        stats = [[item.strip() for item in stat.text.strip().replace(
            ':', ',').split(',')] for stat in summary if stat.find('strong')]
        stats = stats[:8]

        return stats

    def season_history(self):
        seasons = pd.read_html(self.school_url, header=[1],
                               attrs={'id': f'{self.abbreviation}'})[0]
        seasons['W'] = seasons['W'].str.replace('*', '', regex=False)
        seasons['L'] = seasons['L'].str.replace('*', '', regex=False)
        seasons['NCAA Tournament'] = seasons['NCAA Tournament']\
