...
def compare_players(players, stats, total='career'):
    ...
    if not stats:
        if total == 'career':
            comparison = pd.concat([player.career_totals()
                                   for player in players], axis=1)
            ...
            comparison.columns = [player.full_name for player in players]
            comparison.drop(index={'Age', 'Tm', 'Lg', 'Pos',
                            'FG%', '3P%', '2P%', 'eFG%', 'FT%'}, inplace=True)
        elif total == 'post':
            comparison = pd.concat([player.post_season_stats().sum()
                                   for player in players], axis=1)
            comparison.columns = [player.full_name for player in players]
            comparison.drop(index={'Age', 'Tm', 'Lg', 'Pos',
                            'FG%', '3P%', '2P%', 'eFG%', 'FT%'}, inplace=True)
