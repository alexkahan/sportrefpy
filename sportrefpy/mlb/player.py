...
Player names are case-sensitive.'''
    ...
    def regular_season_batting(self, season=None, stat=None):
        ...
        '''
        if not self.pitcher:
            batting = pd.read_html(self.player_url, attrs={
                                   'id': 'batting_standard'})[0]
    ...
    def regular_season_pitching(self, season=None):
        ...
        '''
        if self.pitcher:
            pitching = pd.read_html(self.player_url, attrs={
                                    'id': 'pitching_standard'})[0]
            pitching.dropna(how='any', axis='rows',
                            subset='Year', inplace=True)
            pitching = pitching[~pitching['Year'].str.contains(
                'Yrs|yrs|yr|Avg')]
    ...
    def career_totals_pitching(self, stat=None):
        if self.pitcher:
            reg = pd.read_html(self.player_url, attrs={
                               'id': 'pitching_standard'})[0]
            ...
            career.drop(columns={'Series', 'Rslt', 'Opp',
                        'WPA', 'cWPA'}, inplace=True)
